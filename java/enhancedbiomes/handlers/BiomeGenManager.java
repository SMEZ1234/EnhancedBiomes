...
public class BiomeGenManager 
{
	...
	public static BiomeGenBase[] getHotBiomes(BiomeGenBase[] input)
	{
		...
		for(int b = 0; b < hotBiomes.size(); b++) output[b + input.length] = hotBiomes.get(b);
		return output;		
	}
	...
	public static BiomeGenBase[] getWarmBiomes(BiomeGenBase[] input)
	{
		...
		for(int a = 0; a < input.length; a++) output[a] = input[a];
		for(int b = 0; b < warmBiomes.size(); b++) output[b + input.length] = warmBiomes.get(b);
		return output;		
	}
	...
	public static BiomeGenBase[] getCoolBiomes(BiomeGenBase[] input)
	{
		...
		for(int a = 0; a < input.length; a++) output[a] = input[a];
		for(int b = 0; b < coolBiomes.size(); b++) output[b + input.length] = coolBiomes.get(b);
		return output;		
	}
	...
	public static BiomeGenBase[] getFrozenBiomes(BiomeGenBase[] input)
	{
		...
		for(int a = 0; a < input.length; a++) output[a] = input[a];
		for(int b = 0; b < frozenBiomes.size(); b++) output[b + input.length] = frozenBiomes.get(b);
		return output;		
	}
