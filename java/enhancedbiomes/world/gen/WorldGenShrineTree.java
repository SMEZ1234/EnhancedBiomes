...
public class WorldGenShrineTree extends WorldGenAbstractTree
{
	/**
	...
	public WorldGenShrineTree(boolean p_i2008_1_) {
		super(p_i2008_1_);
	}
			...
			while(j1 <= l) {
				...
				if(d0 > (double) (p_150529_4_ * p_150529_4_)) {
					...
					if(!block1.isAir(worldObj, aint1[0], aint1[1], aint1[2]) && !block1.isLeaves(worldObj, aint1[0], aint1[1], aint1[2])) {
						...
						this.setBlockAndNotifyAdequately(this.worldObj, aint1[0], aint1[1], aint1[2], p_150529_6_, meta + (rand.nextInt(4) == 0 ? 0 : 1));
						++j1;
					}
				}
			}
	...
	void generateLeafNode(int p_76491_1_, int p_76491_2_, int p_76491_3_) {
		...
		for(int i1 = p_76491_2_ + this.leafDistanceLimit; l < i1; ++l) {
			...
			this.func_150529_a(p_76491_1_, l, p_76491_3_, f, (byte) 1, EnhancedBiomesBlocks.leavesJungle, 1);
		}
	}

	void func_150530_a(int[] p_150530_1_, int[] p_150530_2_, Block p_150530_3_, int meta) {
	...
	void generateTrunk() {
		...
		this.func_150530_a(aint, aint1, EnhancedBiomesBlocks.logJungle, 2);
	...
	void generateLeafNodeBases() {
		...
		for(int[] aint = new int[] {this.basePos[0], this.basePos[1], this.basePos[2]}; i < j; ++i) {
			...
			if(this.leafNodeNeedsBase(k)) {
				this.func_150530_a(aint, aint2, EnhancedBiomesBlocks.logJungle, 2);
			}
		}
	}

	/**
	 * Checks a line of blocks in the world from the first coordinate to triplet to the second, returning the distance (in blocks) before a non-air, non-leaf block is encountered and/or the end is
	 * encountered.
	 */
	...
	boolean validTreeLocation() {
		...
		boolean isSoil = block.canSustainPlant(worldObj, basePos[0], basePos[1] - 1, basePos[2], ForgeDirection.UP, (BlockSaplingEnhancedBiomes) EnhancedBiomesBlocks.saplingEB);
		if(!isSoil) {
			return false;
		}
		else {
